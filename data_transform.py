...
for dirname in dirs:
    for mode in modes:
        sum_loads = pd.read_csv ('/home/doktormatte/MA_SciComp/' + dirname + '/' + mode + '/1_red.csv', names = list(range(5)))
        ...
        for num in range(1,53):
            try:                
                df = pd.read_csv ('/home/doktormatte/MA_SciComp/' + dirname + '/' + mode + '/' + str(num) + '_red.csv', names = list(range(5)))                            
                for column in df:
                    if column == 3 or column == 4:
...
for dirname in dirs:
    for mode in modes:
        df = pd.read_csv ('/home/doktormatte/MA_SciComp/' + dirname + '/' + mode + '/sum.csv', names = list(range(5)))
        df_norm = pd.DataFrame()
        for column in df:
            if column == 3 or column == 4:                
                x = df[column]
