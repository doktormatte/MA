...
f = open('/home/doktormatte/Dropbox/Dokumente/Studium/MA_SciComp/Data/Original/acndata_sessions_caltech.json')
data = json.load(f)
sites = set()
...
df_acn = df_acn.drop(columns=['clusterID', 'sessionID', 'siteID', 'spaceID', 'timezone'])
...
df_acn['totalDuration'] = df_acn.apply(lambda row: calc_total_time(row), axis=1)
df_acn = df_acn[df_acn['totalDuration'] > 0.0]
...
df_acn.to_csv('/home/doktormatte/Dropbox/Dokumente/Studium/MA_SciComp/Data/Original/test.csv', encoding='utf-8', index=False)
# sys.exit()
...
# stations = ['2-39-79-381', '2-39-92-436','2-39-95-27','2-39-128-562','2-39-124-22']
...
for stat_name in stations:
    df_stat = df_acn[df_acn['stationID'] == stat_name] 
    counts = len(set(list(df_stat['_id'])))
    counts_df = pd.DataFrame(columns=['num','count','occupancy','check','check1'])
    counts_df['num'] = np.array([str(stat_name)])    
    counts_df['count'] = counts
    counts_df['occupancy'] = np.sum(df_stat.totalDuration)/total_minutes
    # print(counts_df)
...
for stat_name in stations:
    ...
    file_name_load = "/home/doktormatte/MA_SciComp/ACN_1/Loads/" + str(backbone_num) + ".csv"
    ...
    file_name_occup = "/home/doktormatte/MA_SciComp/ACN_1/Occup/" + str(backbone_num) + ".csv"
    ...
    # session_counts[session_counts.num == stat_name].check = len(sorted_backbone_occup[sorted_backbone_occup.value_shifted == 1]) / len(sorted_backbone_occup.value_shifted)
   
    session_counts.loc[session_counts.num == stat_name, 'check'] = len(sorted_backbone_occup[sorted_backbone_occup.value_shifted == 1]) / len(sorted_backbone_occup.value_shifted)
...
session_counts.to_csv('/home/doktormatte/MA_SciComp/ACN_1/Occup/session_counts.csv', encoding='utf-8', index=False)
