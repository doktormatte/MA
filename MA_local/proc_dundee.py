...
df_dundee = pd.read_csv (r'/home/doktormatte/Dropbox/Dokumente/Studium/MA_SciComp/Data/Original/Dundee_all.csv')
...
for stat_name in stations:
    df_stat = df_dundee[df_dundee['CP ID'] == stat_name] 
    counts = len(df_stat)
    counts_df = pd.DataFrame(columns=['num','count','occupancy','check','check1'])
    counts_df['num'] = np.array([stat_name])
    counts_df['count'] = counts
    counts_df['occupancy'] = np.sum(df_stat['Total_Duration'])/total_minutes
    # print(counts_df)
    session_counts = pd.concat([session_counts, counts_df])    
    backbone_num += 1

# session_counts.to_csv('/home/doktormatte/MA_SciComp/Dundee/Occup/session_counts.csv', encoding='utf-8', index=False)
