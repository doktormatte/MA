...
backbone_num = 1
session_counts = pd.DataFrame(columns=['num','count'])
for stat_name in stations:
    df_stat = df_pk[df_pk['CP ID'] == stat_name] 
    counts = len(df_stat)
    counts_df = pd.DataFrame(columns=['num','count'])
    counts_df['num'] = np.array([str(backbone_num)])
    counts_df['count'] = counts
    # print(counts_df)
    session_counts = pd.concat([session_counts, counts_df])    
    backbone_num += 1

session_counts.to_csv('/home/doktormatte/MA_SciComp/Perth_Kinross/Occup/session_counts.csv', encoding='utf-8', index=False)
