...
def calc_sessions_dur(row):
    val = (row['Total kWh']/50.0)*60.0
    
    if val < 15.0:
        return 15.0
    return 15.0*round(val/15.0)   
...
def add_to_backbones(row, stat_name):
    ...
    iters_load = int(row['Session_Duration']/15)


    backbone_load = stat_backbones[stat_name][0]
    for i in range(iters_load):
        backbone_load.loc[backbone_load['date_time'] == row['Start_Timestamp'] + datetime.timedelta(minutes=15*i), 'value'] += row['Load_per_quarter']     
    
    
    iters_occup = int(row['Total_Duration']/15)
    backbone_occup = stat_backbones[stat_name][1]
    for i in range(iters_occup):
        backbone_occup.loc[backbone_occup['date_time'] == row['Start_Timestamp'] + datetime.timedelta(minutes=15*i), 'value'] = 1.0   
...
df_dundee['Start_Timestamp'] = df_dundee['Start_Timestamp'].map(roundTime)

df_dundee['End_Timestamp'] = df_dundee['End_Timestamp'].dt.to_pydatetime()
df_dundee['End_Timestamp'] = df_dundee['End_Timestamp'].map(roundTime)

df_dundee['New_End_Timestamp'] = df_dundee.apply(lambda row: adjust_ts(row), axis=1)
df_dundee['End_Timestamp'] = df_dundee['New_End_Timestamp']


df_dundee['Session_Duration'] = df_dundee.apply(lambda row: calc_sessions_dur(row), axis=1)
# df_dundee['Session_Duration'] = round(df_dundee['Session_Duration'])

df_dundee['Total_Duration'] = df_dundee.apply(lambda row: calc_total_dur(row), axis=1)
# df_dundee['Total_Duration'] = round(df_dundee['Total_Duration'])

# df_dundee = df_dundee[df_dundee['Session_Duration'] > 0.0]
# df_dundee = df_dundee[df_dundee['Total_Duration'] > 0.0]


print(len(df_dundee))
df_dundee = df_dundee[df_dundee['Total_Duration'] >= df_dundee['Session_Duration']]
print(len(df_dundee))

df_dundee['z_score'] = np.abs(stats.zscore(df_dundee['Session_Duration']))
df_dundee = df_dundee[df_dundee.z_score <= 3.0]
...
for stat_name in stations:
    ...
    df_dundee.apply(lambda row: add_to_backbones(row, stat_name), axis=1)        
